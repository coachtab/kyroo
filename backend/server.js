...
function buildProgramPrompt(programId, data) {
  ...
  switch (programId) {
    ...
    // ── 7. Hyrox Race Plan ─────────────────────────────────────────────────
    case 'hyrox': {
      const hxDuration = timeframe || '12 weeks';
      const hxWeeks = parseInt(hxDuration) || 12;
      const hxBase = Math.max(Math.round(hxWeeks * 0.4), 2);
      const hxBuild = Math.max(Math.round(hxWeeks * 0.35), 2);
      return {
        system: `You are an elite Hyrox coach with deep expertise in hybrid fitness racing. You understand the unique demands of Hyrox — 8 rounds of 1km running alternated with 8 functional workout stations (SkiErg, Sled Push, Sled Pull, Burpee Broad Jumps, Rowing, Farmers Carry, Sandbag Lunges, Wall Balls). You write programs that develop the specific strength-endurance, running economy, and station technique needed to race Hyrox well. You NEVER write generic fitness programs — every session is Hyrox-specific.\n\n${KYROO_SYSTEM_RULES}`,
        user: `ATHLETE PROFILE:\n${clientBase}\n- Hyrox goal: ${primary_goal || 'complete a Hyrox race'}\n- Race experience: ${level || 'first race'}\n- Training setup: ${equipment || 'standard gym'}\n- Weakest area: ${nutrition || 'run-station transitions'}\n- Biggest race day concern: ${biggest_challenge || 'fading in the back half'}\n- Weeks to race: ${hxDuration}\n\nCreate a ${hxDuration.toUpperCase()} HYROX RACE PREPARATION PLAN. Every session must develop the specific fitness required for Hyrox racing — running economy under fatigue, station technique, and strength-endurance.\n\nSTRUCTURE YOUR PLAN EXACTLY LIKE THIS:\n\n# KYROO ${hxDuration.toUpperCase()} HYROX RACE PLAN\nDesigned for: ${level} | ${days_per_week} sessions/week | Goal: ${primary_goal} | Race in ${hxDuration}\n\n## HYROX RACE BREAKDOWN\nExplain the race format (8 × 1km run + 8 stations in order). State each station's distance/volume at Open level. Explain why the run-station transition is the hardest part. This is for athletes who may be unfamiliar.\n\n## YOUR WEAKEST ZONE: FIXING "${nutrition}"\n3 targeted workouts and drills specifically for their weakness. Include measurable targets to track weekly improvement.\n\n## YOUR WEEKLY TRAINING STRUCTURE (${days_per_week} sessions/week)\nLabel each session type clearly:\n- RUN SESSION (aerobic base, tempo, or race-pace intervals)\n- STATION SESSION (technique + strength-endurance for Hyrox stations)\n- HYBRID SESSION (run + stations combined — closest to race simulation)\n- STRENGTH SESSION (supporting strength work where needed)\nAll station exercises must use equipment available at: ${equipment || 'standard gym'}. Provide substitutes for any unavailable equipment.\n\n## PHASE 1 — WEEKS 1–${hxBase}: BASE CONDITIONING\nFocus: Build aerobic running base, learn station technique, establish work capacity.\n- Running: easy aerobic runs, learn pacing zones\n- Stations: technique-first, moderate loads, master form before intensity\n- End of phase: baseline time trial — simulate 2 stations + 2km run\n\n## PHASE 2 — WEEKS ${hxBase + 1}–${hxBase + hxBuild}: RACE-SPECIFIC BUILD\nFocus: Increase station loads, introduce threshold running, build run-station transitions.\n- Introduce hybrid workouts (run + stations back to back)\n- Station loads increase to race-weight targets\n- Key session: progressive race simulation (4 stations + 4km)\n\n## PHASE 3 — WEEKS ${hxBase + hxBuild + 1}–${hxWeeks}: PEAK & TAPER\nFocus: Race pace, full simulation, taper.\n- Full race simulation: 8 stations + 8km run (spread over one session)\n- Race pacing strategy: exact split targets per station and per 1km run\n- Taper: reduce volume week ${hxWeeks - 1}–${hxWeeks}, maintain intensity\n- Race week protocol: what to do the 7 days before the race\n\n## RACE DAY STRATEGY\n- Warm-up protocol (what to do 30-60 min before start)\n- Pacing plan: exact target time per 1km run and per station\n- Nutrition and hydration during the race\n- Tactical advice for ${nutrition} — their specific weakness on race day\n\n## NUTRITION FOR HYROX TRAINING\nFuelling for hybrid training: carbohydrates, protein timing, race-day nutrition. Keep it practical and simple.`,
        maxTokens: 9000,
      };
    }

    // ── 8. Marathon Race Plan ───────────────────────────────────────────────
    case 'marathon': {
      const mrDuration = timeframe || '16 weeks';
      const mrWeeks = parseInt(mrDuration) || 16;
      const targetTime = nutrition || 'finish';
      const weeklyMileage = equipment || 'under 20km per week';
      const isHalfMarathon = (primary_goal || '').toLowerCase().includes('half') || (primary_goal || '').includes('21.1');
      const raceDistance = isHalfMarathon ? 'half marathon (21.1km)' : 'full marathon (42.2km)';
      const peakLongRun = isHalfMarathon ? '18-20km' : (mrWeeks >= 16 ? '32-35km' : '28-30km');
      const mrBase = Math.max(Math.round(mrWeeks * 0.35), 3);
      const mrBuild = Math.max(Math.round(mrWeeks * 0.40), 3);
      return {
        system: `You are an elite endurance running coach with 15+ years preparing runners for half marathons and full marathons — from first-time finishers to sub-3-hour athletes. You write evidence-based programs built on progressive long run build, polarised intensity (80% easy, 20% quality), and structured taper. You understand half marathon and full marathon training as distinct disciplines with different peak long run distances, taper protocols, and race-day strategies. You NEVER write generic run plans — every session has a specific purpose.\n\n${KYROO_SYSTEM_RULES}`,
        user: `RUNNER PROFILE:\n${clientBase}\n- Race: ${raceDistance}\n- Goal: ${primary_goal || `complete the ${raceDistance}`}\n- Running background: ${level || 'some race experience'}\n- Current weekly mileage: ${weeklyMileage}\n- Target finish time: ${targetTime}\n- Biggest challenge: ${biggest_challenge || 'pacing and energy management'}\n- Weeks to race: ${mrDuration}\n\nCreate a ${mrDuration.toUpperCase()} ${raceDistance.toUpperCase()} TRAINING PLAN. Every run has a specific purpose. Never write generic fitness content — this is a race-specific program.\n\nSTRUCTURE YOUR PLAN EXACTLY LIKE THIS:\n\n# KYROO ${mrDuration.toUpperCase()} ${isHalfMarathon ? 'HALF MARATHON' : 'MARATHON'} PLAN\nDesigned for: ${level} | ${days_per_week} runs/week | Target: ${targetTime} | Race: ${raceDistance} in ${mrDuration}\n\n## YOUR TRAINING PACES\nCalculate and state ALL paces from the target finish time of ${targetTime} for a ${raceDistance}:\n- Easy / recovery pace (min/km)\n- Long run pace (min/km)\n- Race pace (min/km — this is the goal pace to hold on race day)\n- Tempo / threshold pace (min/km)\n- Interval pace (min/km)\nExplain WHY easy runs must be genuinely easy (80/20 rule). Provide RPE and heart rate zone equivalents.\n\n## WEEKLY RUN TYPES\nDefine every run type in this plan:\n- EASY RUN, LONG RUN, TEMPO RUN, INTERVAL SESSION, RECOVERY RUN\nFor each: what it feels like, how to pace it, why it exists in the plan.\n\n## PHASE 1 — WEEKS 1–${mrBase}: BASE BUILDING\nFocus: Build aerobic base from current mileage (${weeklyMileage}), establish long run routine.\n- Starting long run: appropriate distance based on current base\n- Weekly mileage progression (max 10% per week)\n- Full session breakdown for all ${days_per_week} running days\n\n## PHASE 2 — WEEKS ${mrBase + 1}–${mrBase + mrBuild}: RACE-SPECIFIC BUILD\nFocus: Introduce quality sessions, extend long run, build race fitness.\n- Long run builds towards ${isHalfMarathon ? '16-18km' : (mrWeeks <= 14 ? '28-30km' : '30-32km')} by end of phase\n- Introduce race-pace segments within long runs\n- Tempo and interval sessions written out in full\n- Specific strategy for: "${biggest_challenge}"\n\n## PHASE 3 — WEEKS ${mrBase + mrBuild + 1}–${mrWeeks - 2}: PEAK TRAINING\nFocus: Maximum training load, peak long run, race simulation.\n- Peak long run: ${peakLongRun}\n- Race simulation: hold race pace for a meaningful segment\n${isHalfMarathon ? '- Half marathon-specific: negative split strategy and final 5km surge' : '- Full marathon-specific: wall prevention strategy and km 30-35 mental tactics'}\n\n## TAPER — FINAL 2 WEEKS\nWeek ${mrWeeks - 1}: Cut volume 30-40%, keep intensity\nWeek ${mrWeeks} (Race week): Day-by-day protocol — what to run, eat, and how to sleep\n- Carb-loading: when to start, amounts, food choices\n\n## RACE DAY PLAN\n- Morning routine: wake time, pre-race breakfast, warm-up\n- Pacing strategy split by segment: start / middle / ${isHalfMarathon ? 'final 5km' : 'km 30-35 / final push'}\n- Fuelling: exact gel and drink timing for ${targetTime} target over ${raceDistance}\n- How to handle "${biggest_challenge}" on race day\n\n## INJURY PREVENTION\n15-min strength and mobility routine, 2×/week. Target: IT band, knees, calves, hip flexors. When to rest vs push through.`,
        maxTokens: 9000,
      };
    }
    ...
    // ── 9. CrossFit Program ─────────────────────────────────────────────────
    case 'crossfit': {
      const cfDuration = timeframe || '12 weeks';
      const cfWeeks = parseInt(cfDuration) || 12;
      const cfBase = Math.max(Math.round(cfWeeks * 0.33), 2);
      const cfBuild = Math.max(Math.round(cfWeeks * 0.40), 2);

      const weakDomain = nutrition || 'conditioning';
      return {
        system: `You are an elite CrossFit coach and strength and conditioning specialist who programs for athletes from complete beginners to competitive CrossFit athletes. You write programming that develops all CrossFit domains: strength, gymnastics, and metabolic conditioning (MetCon). You write real WODs in proper CrossFit format (AMRAP, EMOM, For Time, RFT). You always provide full scaling options for every WOD. You understand CrossFit injury patterns (shoulders, wrists, lower back) and program around them. You NEVER write generic gym workouts — every session is CrossFit-specific.\n\n${KYROO_SYSTEM_RULES}`,
        user: `ATHLETE PROFILE:\n${clientBase}\n- CrossFit goal: ${primary_goal || 'improve fitness and conditioning'}\n- Experience level: ${level || 'some CrossFit experience'}\n- Training setup: ${equipment || 'CrossFit box'}\n- Weakest domain: ${weakDomain}\n- Biggest challenge: ${biggest_challenge || 'no structured progression'}\n- Program length: ${cfDuration}\n\nCreate a ${cfDuration.toUpperCase()} CROSSFIT PROGRAM. Every session must follow real CrossFit structure. No generic gym content.\n\nSTRUCTURE YOUR PLAN EXACTLY LIKE THIS:\n\n# KYROO ${cfDuration.toUpperCase()} CROSSFIT PROGRAM\nDesigned for: ${level} | ${days_per_week} days/week | Weak domain: ${weakDomain} | Goal: ${primary_goal || 'fitness and conditioning'}\n\n## CROSSFIT NOTATION GUIDE (explain once)\nDefine clearly: AMRAP, EMOM, For Time, RFT, Rx, Scaled, WOD, MetCon, 1RM, % of 1RM. Give one example WOD written in full notation.\n\n## YOUR TRAINING STRUCTURE (${days_per_week} days/week)\nEach training day has this structure:\n1. WARM-UP (10 min): movement prep, skill activation, raise heart rate\n2. STRENGTH / SKILL WORK (15-20 min): barbell cycling, gymnastics progressions, or strength cycle\n3. WOD / MetCon (varies): the main conditioning piece\n4. ACCESSORY / COOL-DOWN (5-10 min)\n\nLabel each day by its focus: STRENGTH FOCUS / GYMNASTICS FOCUS / CONDITIONING FOCUS / MIXED.\n\n## DOMAIN DEVELOPMENT: ${weakDomain.toUpperCase()}\nDedicated skill work for ${weakDomain} — included every week of the program:\n- Week 1–${cfBase}: foundational movements and technique\n- Week ${cfBase + 1}+: progressive loading or complexity\nFor each skill drill: name, full instructions, sets/reps or time, scaling option.\n\n## PHASE 1 — WEEKS 1–${cfBase}: FOUNDATION\nFocus: Establish movement quality, baseline conditioning, learn programme structure.\nFor every training day: warm-up outline, strength/skill work (full details), WOD written in CrossFit notation, Rx weights, scaling options (2 levels), time cap.\n\n## PHASE 2 — WEEKS ${cfBase + 1}–${cfBase + cfBuild}: BUILD\nFocus: Increase loading, introduce complex WODs, develop weakest domain.\n- Strength: progressive overload from Phase 1\n- WODs: higher volume, more complex movement combinations\n- Benchmark WOD: include one named CF benchmark (e.g. Fran, Grace, Helen) with full context\n\n## PHASE 3 — WEEKS ${cfBase + cfBuild + 1}–${cfWeeks}: PEAK\nFocus: Maximum intensity, test improvements, peak performance.\n- Retest Phase 1 strength numbers — show how to measure progress\n- Peak WODs: longest or most demanding of the program\n- Deload: final week reduced volume, maintain intensity\n\n## PROGRAMMING AROUND "${biggest_challenge}"\n3 specific programming tactics to directly address their challenge. Make it actionable.\n\n## NUTRITION FOR CROSSFIT\nFuelling for high-intensity training. Pre-WOD, post-WOD, rest day eating. Protein targets. Keep it practical.\n\n## SCALING PHILOSOPHY\nHow to choose between Rx and scaled. Why scaling is not failing — it\'s smart training.`,
        maxTokens: 9000,
      };
    }
    ...
    // ── 10. HIIT Program ────────────────────────────────────────────────────
    case 'hiit': {
      const hiDuration = timeframe || '8 weeks';
      const hiWeeks = parseInt(hiDuration) || 8;
      const sessionLength = nutrition || '30 minutes';
      const hiBase = Math.max(Math.round(hiWeeks * 0.4), 1);
      return {
        system: `You are a high-intensity interval training (HIIT) specialist and conditioning coach with deep expertise in interval programming, work-to-rest ratios, and metabolic conditioning. You know every HIIT format — Tabata, AMRAP, EMOM, circuit training, density blocks, and sprint intervals. You write programs that produce measurable cardio and body composition results while managing cumulative fatigue. You understand that HIIT recovery is as important as the sessions themselves. You NEVER write generic cardio plans — every session has a specific format, purpose, and progression.\n\n${KYROO_SYSTEM_RULES}`,
        user: `CLIENT PROFILE:\n${clientBase}\n- HIIT goal: ${primary_goal || 'improve fitness and burn fat'}\n- Fitness level: ${level || 'moderate fitness'}\n- Training environment: ${equipment || 'gym'}\n- Session length: ${sessionLength}\n- Biggest challenge: ${biggest_challenge || 'maintaining intensity'}\n- Program length: ${hiDuration}\n\nCreate a ${hiDuration.toUpperCase()} HIIT PROGRAM. Every session must be a specifically formatted interval workout — not a generic circuit. Each session fits within ${sessionLength}.\n\nSTRUCTURE YOUR PLAN EXACTLY LIKE THIS:\n\n# KYROO ${hiDuration.toUpperCase()} HIIT PROGRAM\nDesigned for: ${level} | ${days_per_week} sessions/week | Session length: ${sessionLength} | Goal: ${primary_goal || 'conditioning and fat loss'}\n\n## HIIT FORMAT GUIDE (explain once)\nDefine each format used in this program:\n- TABATA: 8 rounds × 20s work / 10s rest (4 min total)\n- AMRAP: As Many Rounds/Reps As Possible in a set time\n- EMOM: Every Minute On the Minute — set reps at the start of each minute, rest the remainder\n- CIRCUIT: set number of exercises, set reps or time, rotate through\n- DENSITY BLOCK: max reps in a fixed time window\nExplain work-to-rest ratio and why it changes with fitness level.\n\n## YOUR WORK-TO-REST PROGRESSION\nBased on fitness level (${level}), state starting work-to-rest ratio (e.g. 1:2 for beginners, 1:1 for intermediate, 2:1 for advanced). Show how this improves across the program phases.\n\n## SESSION STRUCTURE (every session fits within ${sessionLength})\n- Warm-up: 3-5 min dynamic movement\n- Main HIIT block: core intervals\n- Cool-down: 2-3 min\nBreak down exactly how many minutes each section gets.\n\n## PHASE 1 — WEEKS 1–${hiBase}: BUILD THE BASE\nFocus: Learn the formats, establish work capacity, longer rest periods.\n- Use only equipment available at: ${equipment || 'gym'}\n- All exercises: name, how-to (1 sentence), modification option\n- All sessions written in full: format, rounds, exercises, work time, rest time\n- Session types: TABATA / AMRAP / EMOM (rotate across days)\n\n## PHASE 2 — WEEKS ${hiBase + 1}–${hiWeeks}: INCREASE INTENSITY\nFocus: Shorter rest, harder exercises, higher rounds, more complex formats.\n- Show exact progression from Phase 1 (less rest, more rounds, harder moves)\n- Introduce at least one new format not used in Phase 1\n- Address "${biggest_challenge}" with a specific programming strategy\n\n## RECOVERY PROTOCOL\nHow many rest days between sessions. Active recovery options. Signs of overtraining from HIIT. How to know when to reduce intensity vs push through.\n\n## PROGRESSION BENCHMARKS\nA simple fitness test to complete on Day 1 and repeat at the end. Show how to measure improvement: time, reps, or rounds completed.`,
        maxTokens: 8000,
      };
    }
