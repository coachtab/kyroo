...
function buildProgramPrompt(programId, data) {
  ...
  switch (programId) {
    ...
    case 'marathon': {
      ...
      // Classic marathon periodisation
      const mrBase = Math.max(Math.round(mrWeeks * 0.35), 3);
      const mrBuild = Math.max(Math.round(mrWeeks * 0.40), 3);
      return {
        system: `You are an elite marathon coach with 15+ years preparing runners for events from sub-3-hour racing to first-time finishers. You write evidence-based marathon training programs built on the principles of progressive long run build, polarised intensity distribution (80% easy, 20% quality), and structured taper. You understand injury prevention for marathon training (IT band, shin splints, plantar fasciitis) and nutrition for endurance events. You NEVER write generic run plans — every session has a specific purpose.\n\n${KYROO_SYSTEM_RULES}`,
        user: `RUNNER PROFILE:\n${clientBase}\n- Marathon goal: ${primary_goal || 'complete the marathon'}\n- Running background: ${level || 'some race experience'}\n- Current weekly mileage: ${weeklyMileage}\n- Target finish time: ${targetTime}\n- Biggest challenge: ${biggest_challenge || 'hitting the wall'}\n- Weeks to race: ${mrDuration}\n\nCreate a ${mrDuration.toUpperCase()} MARATHON TRAINING PLAN. This is a complete, race-specific program — not a generic fitness plan. Every run has a purpose, every week builds on the last.\n\nSTRUCTURE YOUR PLAN EXACTLY LIKE THIS:\n\n# KYROO ${mrDuration.toUpperCase()} MARATHON PLAN\nDesigned for: ${level} | ${days_per_week} runs/week | Target: ${targetTime} | Race in ${mrDuration}\n\n## YOUR TRAINING PACES\nCalculate and state ALL paces based on target time of ${targetTime}:\n- Easy / recovery pace (min/km)\n- Long run pace (min/km)\n- Marathon race pace (min/km)\n- Tempo pace (min/km)\n- Interval pace (min/km)\nExplain WHY easy runs must be genuinely easy (80/20 rule). State heart rate zones as alternatives.\n\n## WEEKLY RUN TYPES\nDefine every run type used in this plan:\n- EASY RUN: what it is, how it should feel, why it matters\n- LONG RUN: purpose, pacing strategy, fuelling during\n- TEMPO RUN: what threshold means, how to execute\n- INTERVAL SESSION: format, rest, purpose\n- RECOVERY RUN: when and why\n\n## PHASE 1 — WEEKS 1–${mrBase}: BASE BUILDING\nFocus: Build aerobic base, increase weekly mileage safely, establish long run.\n- Starting long run distance appropriate for current mileage (${weeklyMileage})\n- Weekly mileage progression (no more than 10% increase per week)\n- Session breakdown for all ${days_per_week} running days each week\n- Key workout: progressive long run with easy effort\n\n## PHASE 2 — WEEKS ${mrBase + 1}–${mrBase + mrBuild}: MARATHON-SPECIFIC BUILD\nFocus: Introduce quality work, extend long run, build race-specific fitness.\n- Long run approaches ${mrWeeks <= 14 ? '28-30km' : '30-32km'} by end of phase\n- Introduce marathon-pace miles within long runs\n- Tempo runs and interval sessions written out in full\n- Address "${biggest_challenge}" with specific training strategies\n\n## PHASE 3 — WEEKS ${mrBase + mrBuild + 1}–${mrWeeks - 2}: PEAK TRAINING\nFocus: Maximum training stress, longest long run, race simulation.\n- Peak long run: ${mrWeeks >= 16 ? '32-35km' : '28-30km'}\n- Race simulation run: marathon pace for extended segment\n- Mental preparation section: hitting the wall and the strategies to beat it\n\n## TAPER — FINAL 2 WEEKS\nWeek ${mrWeeks - 1}: Reduce volume 30-40%, maintain intensity\nWeek ${mrWeeks} (Race week): Full taper protocol — what to run, what to eat, how to sleep\n- Race week daily schedule\n- Carb-loading guidance: when to start, what to eat, how much\n\n## RACE DAY PLAN\n- Morning routine: wake time, breakfast, warm-up\n- Pacing strategy: first 10km, halfway, km 30-35 (the wall zone), final push\n- Fuelling: exact gel/drink timing per km marker for ${targetTime} target\n- How to beat "${biggest_challenge}" on race day specifically\n\n## INJURY PREVENTION\nWeekly strength and mobility (15 min, 2×/week). Key exercises for marathon-specific injury risk: IT band, knees, shins. When to back off vs push through.`,
        maxTokens: 9000,
      };
    }
